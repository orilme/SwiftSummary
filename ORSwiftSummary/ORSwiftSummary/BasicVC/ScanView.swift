...
public class ScanView: UIView {
    ...
    /// 边角位置，默认与边框线同中心点
    public lazy var cornerLocation = CornerLocation.inside
    
    /// 边框线颜色，默认白色
    public lazy var borderLineColor = UIColor.white
    ...
    /// 边角宽度，默认2.0
    public lazy var cornerWidth:CGFloat = 5.0
    ...
    /// 扫描区的y值
    lazy var scanBorderY: CGFloat = screenHeight / 2 - scanBorderHeight + UIScreen.NAVHEIGHT
    ...
    override public func draw(_ rect: CGRect) {
        ...
        self.customNavigation.backgroundColor = .clear
        self.goBackButton.setImage(UIImage(named: "icon_backWhite"), for: .normal)
        self.goBackButton.backgroundColor = .red
        self.titleLabel.text = "扫描二维码"
        self.rightButton.setTitle("相册", for: .normal)
    ...
    lazy var customNavigation: UIView = {
        let view = UIView()
        self.addSubview(view)
        view.snp.makeConstraints({ (make) in
            make.top.left.right.equalTo(self)
            make.height.equalTo(UIScreen.NAVHEIGHT)
        })
        return view
    }()
    ...
    lazy var goBackButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(goBackClick), for: .touchUpInside)
        self.customNavigation.addSubview(button)
        button.snp.makeConstraints({ (make) in
            make.left.equalTo(self.customNavigation)
            make.bottom.equalTo(self.customNavigation.snp.bottom)
    ...
    lazy var titleLabel: UILabel = {
        ...
        self.customNavigation.addSubview(label)
        label.snp.makeConstraints({ (make) in
            make.centerX.equalTo(self.customNavigation)
            make.bottom.equalTo(self.customNavigation.snp.bottom)
    ...
    lazy var rightButton: UIButton = {
        ...
        self.customNavigation.addSubview(button)
        button.snp.makeConstraints({ (make) in
            make.right.equalTo(self.customNavigation).offset(-10)
            make.bottom.equalTo(self.customNavigation.snp.bottom)
            make.width.height.equalTo(40)
        })
        return button
